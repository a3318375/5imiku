...
layui.use(['element', 'layer', 'util', 'form', 'flow'], function () {
    ...
    var flow = layui.flow;
    flow.load({
        elem: '#musicList' //指定列表容器
        , done: function (page, next) { //到达临界点（默认滚动触发），触发下一页
            var lis = [];
            //以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
            $.get('/music/list?pageNumber=' + page, function (res) {
                //假设你的列表返回在data集合中
                layui.each(res.list, function (index, item) {
                    var code = '<div style="padding: 10px;display:inline-block;"><figure class="imghvr-zoom-in">' +
                        '<img src="'+ item.pictureUrl +'" alt="example-image">' +
                        '<figcaption>' +
                        '<h3>歌名：'+ item.musicName +'</h3>' +
                    '<p>歌手：'+ item.singer +'</p>' +
                    '<p>'+ item.musicDescription +'</p>' +
                    '</figcaption><a href="/music/details/'+ item.musicId +'"></a>' +
                        '</figure></div>';
                    lis.push(code);
                });
